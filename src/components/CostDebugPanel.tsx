...
function MeetingRow({
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={() => mode === "cost" && setOpen((v) => !v)}
        disabled={mode === "time"}
        className={`w-full flex items-center gap-3 px-4 py-3 text-left group ${
          mode === "time" ? "cursor-default" : ""
        }`}
      >
          {/* Expand chevron (cost breakdown only) */}
          {mode === "cost" && (
          ...
          {/* Cost or duration badge */}
          <div className="text-right shrink-0">
            {mode === "time" ? (
              <span
                className={`text-sm font-bold tabular-nums ${
                  meeting.isExcluded ? "text-white/20 line-through" : "text-white"
                }`}
              >
                {durationLabel}
              </span>
      ...
      {/* Expanded breakdown (cost mode only) */}
      {open && mode === "cost" && (
        <div className="border-t border-white/[0.06] px-4 pb-4 pt-3 space-y-3">
          {isMatched ? (
            ...
            >
              {!hasAttendeeDetails && meeting.participantCount > 0 ? (
                <div className="flex items-center gap-2">
                  <AlertTriangle size={13} className="text-amber-400 shrink-0" />
                  <p className="text-xs font-semibold text-amber-300">Private meeting</p>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  <AlertTriangle size={13} className="text-amber-400 shrink-0" />
              ...
              <p className="text-[11px] text-white/30 pl-5">
                {!hasAttendeeDetails && meeting.participantCount > 0
                  ? "Attendee emails are hidden for this meeting, so we can't match team members to calculate costs precisely."
...
export default function CostDebugPanel({
  period,
  mode = "cost",
  onClose,
}: CostDebugPanelProps) {
  ...
  const totalCost = activeMeetings.reduce((s, m) => s + (m.teamCost ?? 0), 0);
  const totalMinutes = activeMeetings.reduce(
    ...
    <>
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-white leading-tight">
                {mode === "cost" ? "Cost Breakdown" : "Meeting time breakdown"}
              </p>
          ...
          >
            ...
            <div className="text-center mb-4">
              <p className="text-[11px] uppercase tracking-widest text-white/30 mb-1">
                {mode === "cost" ? "Total cost" : "Total meeting time"}
              </p>
              <p
                ...
                style={
                  mode === "cost"
                    ? {
                        background:
                          "linear-gradient(135deg, hsl(232,60%,75%), hsl(190,70%,65%))",
              ...
              >
                {mode === "cost" ? fmt(totalCost) : totalHours}
              </p>
            </div>
            ...
            {mode === "cost" ? (
              ...
              >
                ...
                <div className="py-3 flex flex-col items-center gap-0.5">
                  <span
                    className={`text-xl font-bold ${unmatched > 0 ? "text-amber-300" : "text-white/30"}`}
                  >
                    {unmatched}
                  </span>
                  <span className="text-[10px] text-white/35 uppercase tracking-wide">
                ...
                <div className="py-3 flex flex-col items-center gap-0.5">
                  <span className="text-xl font-bold text-white">
                    {activeMeetings.length > 0
                      ? Math.round(totalMinutes / activeMeetings.length)
                      : 0}
                    m
                  </span>
                  <span className="text-[10px] text-white/35 uppercase tracking-wide">
            ...
            {/* Match quality bar (cost only) */}
            {mode === "cost" && meetings.length > 0 && (
              <div className="mt-3 space-y-1">
                <div className="flex justify-between text-[10px] text-white/30">
                  <span>Match quality</span>
                  <span>
                    {activeMeetings.length > 0
                      ? Math.round((fullyMatched / activeMeetings.length) * 100)
                      : 0}
                    %
                  </span>
                </div>
                <div className="h-1 rounded-full overflow-hidden bg-white/[0.06]">
                  <div
                    ...
                    style={{
                      ...
                      background:
                        "linear-gradient(90deg, hsl(190,70%,50%), hsl(150,60%,45%))",
                    }}
                  />
                </div>
              </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 [scrollbar-width:thin] [scrollbar-color:hsla(232,42%,53%,0.3)_transparent]">
          ...
          {meetings.map((m, i) => (
            <MeetingRow key={m.id} meeting={m} index={i} mode={mode} />
          ))}
        </div>
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-white/30">
                Matched — real member rates
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-400" />
              <span className="text-[10px] text-white/30">
                Unmatched — no team members found
              </span>
            </div>
          </div>
        )}
