...
async function updateProfileSyncAudit(
  ...
  const payload: {
    last_calendar_sync_status: "success" | "failed";
    last_calendar_sync_error: string | null;
    last_calendar_sync_at?: string;
  } = {
    last_calendar_sync_status: status,
    last_calendar_sync_error: error,
  };

  if (status === "success") {
    payload.last_calendar_sync_at = new Date().toISOString();
  }
  ...
  if (auditError) {
    console.error("sync-calendar: failed to update profile sync audit", {
      profileId,
      error: auditError,
    });
  }
...
export async function syncCalendarForProfile(
  ...
  if (!profile.team_id || !profile.microsoft_refresh_token) {
    await updateProfileSyncAudit(
      admin,
      profile.id,
      "failed",
      "Missing team assignment or Microsoft refresh token.",
    );
  ...
  const decryptedRefreshToken = (() => {
    try {
      return decryptToken(profile.microsoft_refresh_token);
    } catch (err) {
      const message =
        err instanceof Error
          ? err.message
          : "Stored Outlook token could not be decrypted.";
      console.error("sync-calendar: token decrypt failed", {
        profileId: profile.id,
        error: err,
      });
      void updateProfileSyncAudit(admin, profile.id, "failed", message);
  ...
  try {
    const tokenResponse = await refreshCalendarAccessToken(decryptedRefreshToken);
    events = await getCalendarEvents(tokenResponse.access_token, from, to);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown Graph error";
    console.error("sync-calendar: Graph fetch failed", {
      profileId: profile.id,
      error: err,
    });
    await updateProfileSyncAudit(admin, profile.id, "failed", message);
    return { synced: 0, total: 0, deleted: 0, costsRecalculated: 0 };
  }

  if (!events || events.length === 0) {
    await updateProfileSyncAudit(admin, profile.id, "success", null);
    return { synced: 0, total: 0, deleted: 0, costsRecalculated: 0 };
  }
  ...
  for (const event of events) {
    ...
    const { error: upsertError } = await admin.from("meetings").upsert(
      {
        user_id: profile.id,
