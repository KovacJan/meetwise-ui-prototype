...
const AZURE_ENDPOINT = process.env.AZURE_OPENAI_ENDPOINT; // classic: https://my-resource.openai.azure.com — Foundry: https://.../openai/v1
const AZURE_API_KEY = process.env.AZURE_OPENAI_API_KEY;
const AZURE_DEPLOYMENT = process.env.AZURE_OPENAI_DEPLOYMENT ?? "gpt-4o";
const AZURE_API_VERSION_CLASSIC =
  process.env.AZURE_OPENAI_API_VERSION ?? "2024-02-01";
const OPENAI_API_KEY = process.env.OPENAI_API_KEY;

/**
 * If only the Foundry host is pasted (no /openai/v1), the gateway returns 404.
 * e.g. https://xxx.services.ai.azure.com → …/openai/v1
 * Scoped projects keep path: …/api/projects/foo/openai/v1
 */
function normalizeAzureOpenAiEndpoint(raw: string): string {
  ...
  const isFoundryHost =
    host === "services.ai.azure.com" || host.endsWith(".services.ai.azure.com");
  if (!isFoundryHost) return trimmed;

  let path = u.pathname.replace(/\/+$/, "") || "";
  if (path === "/") path = "";
  const hasOpenAiSegment = path.toLowerCase().includes("/openai");
  if (!hasOpenAiSegment) {
    u.pathname = path ? `${path}/openai/v1` : "/openai/v1";
  }
  return u.toString().replace(/\/$/, "");
}

function withOptionalApiVersion(url: string, apiVersion: string | undefined): string {
  if (!apiVersion) return url;
  return `${url}${url.includes("?") ? "&" : "?"}api-version=${encodeURIComponent(apiVersion)}`;
}

/** Foundry /openai/v1/chat/completions rejects many api-version values; strip unless explicitly allowed. */
function stripApiVersionQuery(url: string): string {
  try {
    const u = new URL(url);
    u.searchParams.delete("api-version");
...
/** Foundry-style POST …/chat/completions + JSON body.model (not classic …/deployments/…/chat/completions) */
function isFoundryOpenAICompatibleBase(endpoint: string): boolean {
  const e = endpoint.toLowerCase().replace(/\/$/, "");
  if (e.includes(".openai.azure.com") && !e.includes("/openai/v1")) return false;
  if (e.includes("services.ai.azure.com")) return true;
  if (e.includes("/openai/v1")) return true;
  return process.env.AZURE_OPENAI_USE_FOUNDRY === "1";
}

function azureRequestPathForError(url: string): string {
  try {
    const u = new URL(url);
...
async function chatCompletion(
  ...
  if (isAzureAIConfigured()) {
    const chatOverride = process.env.AZURE_OPENAI_CHAT_COMPLETIONS_URL?.trim();
    const base = normalizeAzureOpenAiEndpoint(AZURE_ENDPOINT!.replace(/\/$/, ""));
    const useFoundry =
      !!chatOverride || isFoundryOpenAICompatibleBase(base);
    ...
    const foundryQueryApiVersion =
      process.env.AZURE_OPENAI_FOUNDRY_API_VERSION?.trim() || undefined;

    const applyFoundryAuth = () => {
      if (process.env.AZURE_OPENAI_AUTH_HEADER === "Bearer") {
    ...
    if (chatOverride) {
      // Full POST URL (e.g. project-scoped gateway). Same JSON body as OpenAI SDK chat.completions.
      url = stripApiVersionQuery(chatOverride.replace(/\/$/, ""));
      if (foundryQueryApiVersion) {
        url = withOptionalApiVersion(url, foundryQueryApiVersion);
      }
      body = {
      ...
      url = stripApiVersionQuery(`${base}/chat/completions`);
      if (foundryQueryApiVersion) {
        url = withOptionalApiVersion(url, foundryQueryApiVersion);
      }
      body = {
      ...
      url = `${base}/openai/deployments/${AZURE_DEPLOYMENT}/chat/completions?api-version=${AZURE_API_VERSION_CLASSIC}`;
      headers["api-key"] = AZURE_API_KEY!;
    ...
    if (!res.ok) {
      ...
      const path = azureRequestPathForError(url);
      let hint = "";
      if (res.status === 404) {
        if (useFoundry) {
          hint =
            " Hint (Foundry): check AZURE_OPENAI_DEPLOYMENT matches the model name, or AZURE_OPENAI_AUTH_HEADER=Bearer.";
        } else {
          hint =
            " Hint (classic Azure): deployment name must exist on this resource. For Azure AI Foundry use AZURE_OPENAI_ENDPOINT ending with /openai/v1 (see .env.example).";
        }
      } else if (
        res.status === 400 &&
        useFoundry &&
        /api version not supported/i.test(text)
      ) {
        hint =
          " Hint (Foundry): do not set AZURE_OPENAI_API_VERSION on the v1 chat URL; remove it in Vercel or use AZURE_OPENAI_FOUNDRY_API_VERSION only if your gateway docs require a specific api-version.";
      }
      throw new Error(`Azure OpenAI error (${res.status}) ${path}: ${text}${hint}`);
    }
