...
async function sumCostsForRange(
  ...
  const {data, error} = await supabase
    .from("meetings")
    .select("id, cost, team_cost, start_time, ical_uid, series_master_id, title, duration_minutes")
    .eq("team_id", teamId)
  ...
  const canonicalRows = dedupeCanonicalMeetings(data);
  // Only use team_cost (from team members with hourly rate). No fallback to sync cost.
  return canonicalRows.reduce((sum, row) => sum + (row.team_cost ?? 0), 0);
}
