...
export async function POST(req: Request) {
  try {
    ...
    if (!profile?.team_id) {
      return Response.json({error: "No team assigned"}, {status: 403});
    }
    ...
    if (uniqueMembers.length === 0) {
      return Response.json({error: "No valid email addresses"}, {status: 400});
    }

    if (
      uniqueMembers.length >
      RATE_LIMIT_POLICIES.invite.maxEmailsPerRequest
    ) {
      return rateLimitValidationResponse("RATE_LIMIT_INVITE_BATCH");
    }

    const burstLimit = await checkRateLimit("invite.userBurst", user.id);
    if (!burstLimit.success) {
      return rateLimitedResponse(
        "RATE_LIMIT_INVITE_BURST",
        burstLimit.retryAfterSeconds ?? 60,
      );
    }

    const teamDailyLimit = await checkRateLimit(
      "invite.teamDaily",
      profile.team_id,
      {cost: uniqueMembers.length},
    );
    if (!teamDailyLimit.success) {
      return rateLimitedResponse(
        "RATE_LIMIT_INVITE_DAILY",
        teamDailyLimit.retryAfterSeconds ?? 86400,
      );
    }
