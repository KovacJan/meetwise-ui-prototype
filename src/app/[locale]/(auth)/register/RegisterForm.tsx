...
export function RegisterForm({
  ...
  const isFormValid =
    !validateEmail(email) && !validatePassword(password) && acceptLegal;
          ...
          {devConfirmUrl && (
            <div className="mb-4 rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-left">
              ...
              <p className="text-xs text-muted-foreground mb-2">
                Click the link below to confirm your account directly (only
        ...
        <form action={formAction}>
          ...
          {teamCode && (
            <p className="mt-4 text-xs text-muted-foreground">
              You&apos;re joining team code{" "}
              <span className="font-mono">{teamCode}</span> after signup.
            </p>
          )}

          <div className="mt-4 rounded-xl border border-border/80 bg-background/30 px-3.5 py-3">
            <label className="flex items-start gap-2.5 text-xs text-muted-foreground leading-relaxed cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (legalTouched && e.target.checked) setLegalTouched(false);
                }}
                onBlur={() => setLegalTouched(true)}
                className="mt-0.5 h-4 w-4 rounded border-border bg-input text-secondary focus:ring-ring"
              />
              <span>
                {t("acceptLegalPrefix")}{" "}
                <Link
