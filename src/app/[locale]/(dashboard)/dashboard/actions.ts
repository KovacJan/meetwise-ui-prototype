  ...
  if (profile.is_manager) {
    ...
    const trendData = await (async () => {
      ...
      for (const row of data ?? []) {
        ...
        buckets.set(weekLabel, (buckets.get(weekLabel) ?? 0) + (row.team_cost ?? 0));
      }
...
export async function getMeetingsForPeriod(
  ...
  let query = admin
    .from("meetings")
    .select(
      "id, title, start_time, end_time, duration_minutes, participant_count, attendees, team_cost, cost, is_cancelled, series_master_id, ical_uid, is_excluded",
    )
  ...
  const typedData = (data ?? []) as MeetingQueryRow[];
  const rowsForView = profile.is_manager
    ? dedupeCanonicalMeetings<MeetingQueryRow>(typedData)
    : typedData;
...
export async function getCostTrendForPeriod(
  ...
  const {data: meetingsData} = await meetingsQuery.order("start_time", {ascending: true});
