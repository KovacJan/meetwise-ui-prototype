...
const Upgrade = () => {
  ...
  const handleUpgrade = async () => {
    if (!isManager) {
      setError("Only team leaders can upgrade the team plan.");
      return;
    }
    setError(null);
    setLoading(true);
      ...
      <div className="w-full max-w-3xl">
        ...
        {!isManager && (
          <div className="mb-6 rounded-xl border border-border/60 bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
            Only team leaders can change billing plans. Ask your team leader to upgrade.
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {displayPlans.map((planItem) => (
            ...
            >
              ...
              <button
                type="button"
                  disabled={planItem.active || loading || !isManager}
