      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {showFinancials && rec.savings != null && (
...
function ProDashboard({t}: {t: ReturnType<typeof useTranslations>}) {
  ...
  const showFinancials = isManager;
  ...
  const generateRecommendations = async () => {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        totalCost: showFinancials ? data.kpis.totalCost : 0,
  ...
  return (
    <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 overflow-y-auto overflow-x-hidden pb-24 md:pb-6 lg:pb-8">
      ...
      >
        {isLoading ? (
          Array.from({length: showFinancials ? 4 : 3}).map((_, i) => (
          ...
          <>
            {showFinancials ? (
              ...
              <KpiTile
                title={t("kpiTimeTitle")}
                value={
                  data
                    ? formatDurationMinutes(
                        data.kpis.totalMinutes ?? 0,
                        locale,
                      )
                    : "—"
                }
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr,380px] gap-6">
        ...
        <div>
          ...
          <HowItWorksPanel t={t} showFinancials={showFinancials} />
